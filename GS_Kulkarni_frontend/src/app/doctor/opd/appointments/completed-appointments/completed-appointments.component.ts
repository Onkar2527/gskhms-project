...
export class CompletedAppointmentsComponent
...
// toggleSpecialCase(row: any): void {
//   if (row.isSpecialCase) {
//     this.specialCases.push(row);
//   } else {
//     this.specialCases = this.specialCases.filter(r => r.id !== row.id);
//   }

//   console.log('Selected special cases:', this.specialCases);
// }

// toggleSpecialCase(row: any): void {
//   const updatedData = {
//     is_special_case: row.isSpecialCase // true if checked, false if unchecked
//   };

//   this.appointmentsService.specialCase(row.id, updatedData).subscribe({
//     next: (res) => {
//       console.log('Special case status updated:', res);
//       // Optional: show toast message or refresh view
//     },
//     error: (err) => {
//       console.error('Failed to update special case status:', err);
//     }
//   });
// }
...
toggleSpecialCase(row: any): void {
  const dialogRef = this.dialog.open(DialogboxComponent, {
    data: {
      message: 'Mark this appointment as a Special Case?'
    }
  });

  dialogRef.afterClosed().subscribe(result => {
    if (result === true) {
      const updateData = { is_special_case: true };

      this.appointmentsService.specialCase(row.id, updateData).subscribe({
        next: () => {
          row.isSpecialCase = true; // update UI state
          this.snackBar.open('Marked as special case', 'Close', { duration: 2000 });
        },
        error: (err) => {
          console.error(err);
          this.snackBar.open('Update failed. Please try again.', 'Close', { duration: 2000 });
        }
      });
    }
  });
