...
export class RoomListComponent extends UnsubscribeOnDestroyAdapter
implements OnInit {
displayedColumns = [
 // 'select',
 'id',
...
addNew() {
  ...
  this.router.navigateByUrl('/admin/master/add-room', { state: { roomData: { id: 0} } });
}
editCall(row: Room) {
  ...
  let tempDirection: Direction;
  this.router.navigateByUrl('/admin/master/add-room', {state: {roomData: row}});
}
...
connect(): Observable<Room[]> {
  ...
  this.exampleDatabase.getRoomList({});
  return merge(...displayDataChanges).pipe(
    map(() => {
      // Filter data
      this.filteredData = this.exampleDatabase.data
        .slice()
        .filter((routeList: Room) => {
          const searchStr = (
            routeList.id +
            routeList.name
          ).toLowerCase();
