...
export class UnitListComponent extends UnsubscribeOnDestroyAdapter
implements OnInit {
displayedColumns = [
 // 'select',
 'id',
...
addNew() {
  ...
  this.router.navigateByUrl('/admin/master/add-unit', { state: { unitData: { id: 0} } });
}
editCall(row: Unit) {
  ...
  let tempDirection: Direction;
  this.router.navigateByUrl('/admin/master/add-unit', {state: {unitData: row}});
}
...
export class ExampleDataSource extends DataSource<Unit> {
...
connect(): Observable<Unit[]> {
  ...
  this.exampleDatabase.getUnitList({});
  return merge(...displayDataChanges).pipe(
    map(() => {
      // Filter data
      this.filteredData = this.exampleDatabase.data
        .slice()
        .filter((routeList: Unit) => {
          const searchStr = (
            routeList.id +
            routeList.name
          ).toLowerCase();
