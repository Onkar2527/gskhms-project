...
@Component({
  ...
  styleUrl: './add-operation-type.component.scss',
  imports: [
    SharedModule
  ]
})
export class AddOperationTypeComponent {
  ...
  panleTitle: string = 'Add Operation Type';
  templateForm: any;
  id: any;
  template: OperationType = new OperationType();
  formdata = {
    id: null,
    name: null
  };
  ...
  constructor(private fb: UntypedFormBuilder,
    ...
    if (urlData?.templateData) {
      ...
      this.template = <OperationType>urlData.templateData;
  ...
  ngOnInit(): void {
    const req = new OperationType();
    req.id = this.id;
    if(this.id){
      this.panleTitle = 'Edit Operation Type';
    }
  ...
  onSubmit() {
    this.masterService.addoperationType(this.templateForm.value).subscribe((result: any) => {
      if (result['message'] == 'Data Saved Successfully') {
        this.openSnackBar(result['message']);
        this.router.navigate(['/admin/master/operation-type-list']);
      } else {
        this.openSnackBar(result['message']);
      }
    });
