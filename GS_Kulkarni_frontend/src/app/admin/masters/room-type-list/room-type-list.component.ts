...
export class RoomTypeListComponent extends UnsubscribeOnDestroyAdapter
implements OnInit {
displayedColumns = [
 // 'select',
 'id',
...
addNew() {
  ...
  this.router.navigateByUrl('/admin/master/add-room-type', { state: { roomTypeData: { id: 0} } });
}
editCall(row: RoomType) {
  
  this.id = row.id;
  console.log("update Route", this.id);
  let tempDirection: Direction;
  this.router.navigateByUrl('/admin/master/add-room-type', {state: {roomTypeData: row}});
}
...
export class ExampleDataSource extends DataSource<RoomType> {
...
connect(): Observable<RoomType[]> {
  ...
  this.exampleDatabase.getRoomTypeList({});
  return merge(...displayDataChanges).pipe(
    map(() => {
      // Filter data
      this.filteredData = this.exampleDatabase.data
        .slice()
        .filter((routeList: RoomType) => {
          const searchStr = (
            routeList.id +
            routeList.name
          ).toLowerCase();
...
sortData(data: RoomType[]): RoomType[] {
