...
export class FloorListComponent extends UnsubscribeOnDestroyAdapter
implements OnInit {
displayedColumns = [
 // 'select',
 'id',
...
addNew() {
  ...
  this.router.navigateByUrl('/admin/master/add-floor', { state: { floorData: { id: 0} } });
}
editCall(row: Floor) {
  ...
  let tempDirection: Direction;
  this.router.navigateByUrl('/admin/master/add-floor', {state: {floorData: row}});
}
...
export class ExampleDataSource extends DataSource<Floor> {
...
connect(): Observable<Floor[]> {
  ...
  this.exampleDatabase.getFloorList({});
  return merge(...displayDataChanges).pipe(
    map(() => {
      // Filter data
      this.filteredData = this.exampleDatabase.data
        .slice()
        .filter((routeList: Floor) => {
          const searchStr = (
            routeList.id +
            routeList.name
          ).toLowerCase();
