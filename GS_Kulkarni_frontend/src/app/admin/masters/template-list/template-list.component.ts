...
addNew() {
  ...
  this.router.navigateByUrl('/admin/master/add-template', { state: { templateData: { id: 0} } });
}
editCall(row: Template) {
  ...
  let tempDirection: Direction;
  this.router.navigateByUrl('/admin/master/add-template', {state: {templateData: row}});
}
...
connect(): Observable<Template[]> {
  ...
  this.exampleDatabase.getTemplateList({});
  return merge(...displayDataChanges).pipe(
    map(() => {
      // Filter data
      this.filteredData = this.exampleDatabase.data
        .slice()
        .filter((routeList: Template) => {
          const searchStr = (
            routeList.id +
            routeList.name
          ).toLowerCase();
