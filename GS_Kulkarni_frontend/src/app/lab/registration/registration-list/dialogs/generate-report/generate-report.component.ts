...
export class GenerateReportComponent  implements OnInit{
  ...
  evaluateResults(ele: any, dtl : any){
    let tempObj: any = {};
    let objArray: any[] = [];
    tempObj.expression = dtl.formula.toLowerCase();
    for(let parameterDetails of ele.labTestDetailsList){
      let newObj: any = {};
      newObj.key = parameterDetails.serviceDetailedName.replaceAll(/[^a-zA-Z0-9]/g,'').replaceAll(' ','').toLowerCase();
      newObj.value = parameterDetails.value;
      objArray.push(newObj);
    }
    tempObj.keyValueList = objArray;
    this.registrationService.evaluateFormula(tempObj).subscribe((res:any)=>{
      dtl.value = res;
    });
  }
