...
import { ApproveLabReportComponent } from '../registration-list/dialogs/approve-lab-report/approve-lab-report.component';

@Component({
  selector: 'waiting-for-approval',
...
export class WaitingForApprovalComponent
  ...
  editCall(row: Registration,called:any='update') {
    ...
    const dialogRef = this.dialog.open(ApproveLabReportComponent, {
      data: {
        registration: row,
        action: called,
      },
...
export class ExampleDataSource extends DataSource<Registration> {
  ...
  connect(): Observable<Registration[]> {
    ...
    this.exampleDatabase.getAllRegistration({type:'T',approvalStatus:'P',status:"Report Generated", registrationStartDate: this.startDate, registrationEndDate: this.endDate});
