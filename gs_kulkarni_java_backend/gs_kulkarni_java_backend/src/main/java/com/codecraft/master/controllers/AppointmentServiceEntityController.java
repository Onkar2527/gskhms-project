...
@Slf4j
public class AppointmentServiceEntityController {

	@Autowired
	AppointmentServicesService appointmentServicesService;

	@PostMapping(value = "/appointmentservice/search")
	...
	public ResponseEntity<MasterManagerResponse> search(@RequestBody AppointmentServiceEntity appointmentServiceEntity) {
		if (Objects.nonNull(appointmentServiceEntity)) {
			return new ResponseEntity<>(appointmentServicesService.search(appointmentServiceEntity), HttpStatus.OK);
		} else {
			throw new MasterManagerException(MasterConstant.EMPTY_BODY, HttpStatus.BAD_REQUEST);
		}
	}
	
	@PostMapping(value = "/appointmentservice/save")
	@Operation(security = @SecurityRequirement(name = "bearerAuth"))
	public ResponseEntity<MasterManagerResponse> save(@RequestBody AppointmentServiceEntity appointmentServiceEntity) {
		return new ResponseEntity<>( appointmentServicesService.save(appointmentServiceEntity), HttpStatus.OK);
	}


	@DeleteMapping(value = "/appointmentservices/{id}")
	@Operation(security = @SecurityRequirement(name = "bearerAuth"))
	public ResponseEntity<MasterManagerResponse> delete(@PathVariable Integer id) {
		return new ResponseEntity<>( appointmentServicesService.delete(id), HttpStatus.OK);
	}
}
