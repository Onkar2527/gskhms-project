...
import com.codecraft.master.repositories.AnesthesiaTypeRepository;
...
@Service
@Slf4j
public class AnesthesiaTypeService {

    @Autowired
    AnesthesiaTypeRepository repository;

    public MasterManagerResponse search(AnesthesiaType reqDomain) {
        log.info("OperationTypeService : search() started reqDomain {}", reqDomain);
        try {
            List<AnesthesiaType> operationTypes = repository.findByHospitalId(UserContext.getHospitalId());
    ...
    @Transactional
    public MasterManagerResponse save(AnesthesiaType reqDomain) {
        log.info("OperationTypeService : save() started reqDomain {}", reqDomain);
        try {
            reqDomain.setActiveInd(1);
            return new MasterManagerResponse(MasterConstant.DATA_SAVED, repository.save(reqDomain));
        } catch (Exception e) {
    ...
    @Transactional
    public MasterManagerResponse update(AnesthesiaType reqDomain) {
